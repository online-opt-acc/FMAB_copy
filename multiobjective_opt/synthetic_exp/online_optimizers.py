...
class RFTL(BaseOnlineOptimizer):
    def __init__(
        self, function, w_generator: WGenerator, eta=0.1, projection_function=lambda x: x
    ):
        """
        params:
    ...
    def run(self, x0, bounds=None):
        ...
        reg_function = JaxFunc(
            lambda x: 0.5 * jnp.vdot(x, x),
        )

        x = self.minimize(x0, reg_function, jac=reg_function.grad, bounds=bounds)

        for _, w_t in tqdm(enumerate(self.w_generator(), start=1)):
            f_val = self.f(w_t, x)
            loss_t = f_val
            grad_t = self.f.grad(w_t, x)
            ...
            func_t = JaxFunc(
                lambda x, grad_accumulator=grad_accumulator: self.eta
                * jnp.vdot(grad_accumulator, x)
                + reg_function(x)
            )
...
class RFTL_structured(RFTL):
    def run(self, x0, vector_func, bounds=None):
        losses = []
        w_sum: np.array = np.zeros(self.w_dim)
        ...
        for _, w_t in tqdm(enumerate(self.w_generator(), start=1)):
            ...
            func_t = JaxFunc(
                lambda x, w_sum=w_sum: self.eta * jnp.vdot(w_sum, vector_func(x))
                + reg_function(x)
            )
            grad_f_t = func_t.grad
...
class BanditGradDescent(BaseOnlineOptimizer):
    ...
    def run(self, x0: dict, T):
        ...
        for _ in range(T):
            # select arm
            ucbs = [
                values[i] - self.bound_scalers[i] * G * (1 / pulls[i] ** 0.5)
